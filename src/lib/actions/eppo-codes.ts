...
export async function updateFormulationCrop(params: {
  ...
  if (params.includeChildren !== undefined) updateData.include_children = params.includeChildren;
  if (params.isExcluded !== undefined) updateData.is_excluded = params.isExcluded;
...
export async function removeFormulationCrop(formulationId: string, eppoCodeId: string) {
  const supabase = await createClient();
...
export async function getFormulationCrops(formulationId: string) {
  ...
  const { data, error } = await supabase
    .rpc("get_formulation_crops", { f_id: formulationId });
...
export async function removeFormulationTarget(formulationId: string, eppoCodeId: string) {
  const supabase = await createClient();
...
export async function getFormulationTargets(formulationId: string) {
  ...
  const { data, error } = await supabase
    .rpc("get_formulation_targets", { f_id: formulationId });
...
export async function updateUseGroupCrop(params: {
  ...
  if (params.includeChildren !== undefined) updateData.include_children = params.includeChildren;
  if (params.isExcluded !== undefined) updateData.is_excluded = params.isExcluded;
  if (params.isCritical !== undefined) updateData.is_critical = params.isCritical;
  updateData.updated_at = new Date().toISOString();
...
export async function removeUseGroupCrop(useGroupId: string, eppoCodeId: string) {
  const supabase = await createClient();
...
export async function getUseGroupCrops(useGroupId: string) {
  ...
  const { data, error } = await supabase
    .rpc("get_use_group_crops", { ug_id: useGroupId });
...
export async function removeUseGroupTarget(useGroupId: string, eppoCodeId: string) {
  const supabase = await createClient();
...
export async function getUseGroupTargets(useGroupId: string) {
  ...
  const { data, error } = await supabase
    .rpc("get_use_group_targets", { ug_id: useGroupId });
...
export async function removeReferenceProductCrop(referenceProductId: string, eppoCodeId: string) {
  const supabase = await createClient();
...
export async function removeReferenceProductTarget(referenceProductId: string, eppoCodeId: string) {
  const supabase = await createClient();
...
export async function getFormulationCountryCrops(formulationCountryId: string) {
  ...
  const { data, error } = await supabase
    .rpc("get_formulation_country_crops", { fc_id: formulationCountryId });
...
export async function getFormulationCountryTargets(formulationCountryId: string) {
  const supabase = await createClient();
  
  const { data, error } = await supabase
    .rpc("get_formulation_country_targets", { fc_id: formulationCountryId });
...
export async function getFormulationCropsAudit(formulationId: string, limit: number = 50) {
  const supabase = await createClient();
...
export async function getFormulationTargetsAudit(formulationId: string, limit: number = 50) {
  const supabase = await createClient();
