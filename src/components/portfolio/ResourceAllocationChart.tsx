  ...
  businessCases.forEach((bc) => {
    ...
    if (!formulationStats[code]) {
      formulationStats[code] = {
        ...
        startYear: 9999,
        name:
          (("formulation_name" in bc
            ? (bc as any).formulation_name
            : "product_name" in bc
              ? (bc as any).product_name
              : null) as string | null) || code,
      };
    }
  ...
  const data = Object.entries(formulationStats)
    .map(([code, stats]) => {
      ...
      return {
        ...
        name: stats.name,
        x:
          stats.startYear === 9999 ? new Date().getFullYear() : stats.startYear,
