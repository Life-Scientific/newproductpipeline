      ...
      return (
        <div className="bg-popover border border-border rounded-md shadow-md p-2 text-sm">
          ...
          >
            ...
            {currencySymbol}
            {Math.abs(data.valueDisplay).toLocaleString()}
            {!data.isTotal && !data.isPositive && " (decrease)"}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            Cumulative: {currencySymbol}
            {(
              data.pv + (data.isPositive || data.isTotal ? data.uv : 0)
            ).toLocaleString()}
          </p>
        </div>
      );
        ...
        >
          ...
          <Bar dataKey="uv" stackId="a" radius={[2, 2, 2, 2]}>
            {processedData.map((entry, index) => {
              ...
              if (entry.isTotal)
                fill = "var(--color-chart-5)"; // Total column color
