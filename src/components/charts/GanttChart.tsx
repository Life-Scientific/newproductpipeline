...
export function GanttChart({
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-popover border border-border rounded-md shadow-md p-2 text-sm">
          ...
          <p className="text-muted-foreground">Start: {data.startDateStr}</p>
          <p className="text-muted-foreground">End: {data.endDateStr}</p>
          {data.status && <p className="text-xs mt-1">Status: {data.status}</p>}
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      <ResponsiveContainer
        width="100%"
        height={Math.max(height, tasks.length * 40 + 60)}
      >
        <BarChart
          layout="vertical"
          data={data}
        ...
        >
          ...
          >
            {data.map((entry, index) => {
              ...
              if (entry.status === "Completed" || entry.progress === 100)
