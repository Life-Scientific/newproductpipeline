...
export function CountryUseGroups({ useGroups, countryName }: CountryUseGroupsProps) {
  ...
  const columns = useMemo<ColumnDef<FormulationCountryUseGroup>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!crops) return <span className="text-sm text-muted-foreground">—</span>;
        const cropsArr = crops.split(",").map(c => c.trim()).slice(0, 3);
  ...
  return (
    <Card>
      <CardHeader className="space-y-1.5">
        ...
        <CardDescription>
          {useGroups.length} use group{useGroups.length !== 1 ? "s" : ""} across formulations
        </CardDescription>
        {statusCounts.size > 0 && (
          <div className="flex flex-wrap gap-2 mt-2">
            {Array.from(statusCounts.entries()).map(([status, count]) => (
              <Badge key={status} variant={getStatusVariant(status, 'registration')} className="text-xs">
                {status}: {count}
              </Badge>
            ))}
          </div>
