...
import { Loader2, AlertCircle, ExternalLink, FlaskConical, Flame, CheckCircle2, XCircle, AlertTriangle, ChevronDown, ChevronUp } from "lucide-react";
...
function calculateDrugLikeness(data: PubChemEnrichmentData): DrugLikenessProfile | null {
  const { molecularWeight, xlogP, hBondDonorCount, hBondAcceptorCount, tpsa, rotatableBondCount } = data;
  ...
  const rotBondsOk = rotatableBondCount === undefined || rotatableBondCount <= 10;
  const veberCompliant = tpsaOk && rotBondsOk;

  // Calculate overall score (weighted)
  // Lipinski: 60%, Veber: 40%
  const lipinskiPct = (lipinskiScore / 4) * 60;
  const veberPct = veberCompliant ? 40 : (tpsaOk ? 20 : 0) + (rotBondsOk ? 20 : 0);
  ...
  return { lipinskiScore, lipinskiViolations: violations, veberCompliant, overallScore, interpretation };
}

// Expandable Safety Warning Component
function SafetyWarningSection({ signalWord, hazardStatements }: { signalWord: string; hazardStatements: string[] }) {
        ...
        <CollapsibleTrigger asChild>
          <button 
            className={`w-full p-3 flex items-start gap-2.5 text-left ${
              isDanger ? 'text-red-800 hover:bg-red-100/50' : 'text-amber-800 hover:bg-amber-100/50'
            } transition-colors`}
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {hasMultiple && (
                  <Badge variant="outline" className={`text-[10px] h-4 px-1.5 ${
                    isDanger ? 'border-red-300 text-red-700' : 'border-amber-300 text-amber-700'
                  }`}>
              ...
              {hazardStatements[0] && !isExpanded && (
                <p className="text-xs opacity-80 mt-0.5 truncate">{hazardStatements[0]}</p>
              )}
            </div>
            {hasMultiple && (
              isExpanded ? (
        ...
        <CollapsibleContent>
          <div className={`px-3 pb-3 pt-0 space-y-1.5 border-t ${
            ...
            {hazardStatements.map((statement, idx) => (
              <p key={idx} className={`text-xs ${isDanger ? 'text-red-700' : 'text-amber-700'}`}>
                • {statement}
              </p>
            ))}
          </div>
...
export function ChemicalEnrichment({ casNumber, ingredientName }: ChemicalEnrichmentProps) {
  ...
  useEffect(() => {
    ...
    async function fetchEnrichmentData() {
      ...
      try {
        ...
        const [propertiesRes, synonymsRes, experimentalRes, safetyRes] = await Promise.all([
          fetch(
        ...
        if (propertiesRes.ok) {
          ...
          if (p) {
            ...
            enrichmentData.molecularWeight = p.MolecularWeight ? Number(p.MolecularWeight) : undefined;
            enrichmentData.xlogP = p.XLogP !== undefined ? Number(p.XLogP) : undefined;
            enrichmentData.exactMass = p.ExactMass ? Number(p.ExactMass) : undefined;
            enrichmentData.tpsa = p.TPSA !== undefined ? Number(p.TPSA) : undefined;
            ...
            enrichmentData.complexity = p.Complexity ? Number(p.Complexity) : undefined;
        ...
        if (experimentalRes?.ok) {
          try {
            ...
            const findValue = (heading: string) => {
              const section = sections.find((s: any) => s.TOCHeading === heading);
              return section?.Information?.find((i: any) => i.Value?.StringWithMarkup?.[0]?.String)?.Value?.StringWithMarkup?.[0]?.String;
            };
        ...
        if (safetyRes?.ok) {
          try {
            ...
            const ghsSection = sd.Record?.Section?.[0]?.Section?.[0]?.Section?.[0];
            if (ghsSection?.Information) {
              const signalInfo = ghsSection.Information.find((i: any) => i.Name === "Signal");
              enrichmentData.ghsSignalWord = signalInfo?.Value?.StringWithMarkup?.[0]?.String;
              const hazards = ghsSection.Information
                .filter((i: any) => i.Name === "GHS Hazard Statements")
                .map((i: any) => i.Value?.StringWithMarkup?.[0]?.String)
                .filter(Boolean);
              if (hazards.length) enrichmentData.ghsHazardStatements = hazards.slice(0, 10); // Get more hazard statements
            }
          } catch {}
        }
  ...
  const drugLikeness = useMemo(() => data ? calculateDrugLikeness(data) : null, [data]);
      ...
      <Card>
        <CardContent className="py-12">
          <div className="flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-muted-foreground">Loading PubChem data...</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <Card>
        <CardContent className="pt-6">
          <Alert variant="destructive">
            ...
            <AlertDescription className="text-xs">Unable to load chemical data.</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    );
  ...
  const pubchemUrl = data.cid ? `https://pubchem.ncbi.nlm.nih.gov/compound/${data.cid}` : null;
      ...
      <Card className="overflow-hidden">
        <CardHeader className="bg-muted/30 pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-semibold">Chemical Profile</CardTitle>
            {pubchemUrl && (
          ...
          <div className="p-4 space-y-4">
            ...
            {drugLikeness && (
              <div className="bg-muted/30 rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-muted-foreground">Bioavailability Profile</span>
                  <Badge variant={
                    drugLikeness.interpretation === "Excellent" ? "default" :
                    drugLikeness.interpretation === "Good" ? "secondary" :
                    drugLikeness.interpretation === "Moderate" ? "outline" : "destructive"
                ...
                <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                    ...
                    <TooltipContent side="bottom" className="text-xs max-w-[200px]">
                      ...
                      {drugLikeness.lipinskiViolations.length === 0 ? (
                        ...
                        <ul className="text-red-600">
                          {drugLikeness.lipinskiViolations.map((v, i) => <li key={i}>• {v}</li>)}
                        </ul>
                      )}
                    </TooltipContent>
                  </Tooltip>
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" className="text-xs">
                      ...
                      {data.tpsa !== undefined && <p>TPSA: {data.tpsa.toFixed(1)} Å²</p>}
                      {data.rotatableBondCount !== undefined && <p>Rot. bonds: {data.rotatableBondCount}</p>}
                    </TooltipContent>
                  </Tooltip>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="font-mono font-medium">{data.molecularFormula || "—"}</p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Weight</p>
                <p className="font-medium">{data.molecularWeight ? `${data.molecularWeight.toFixed(2)} g/mol` : "—"}</p>
              </div>
              {data.xlogP !== undefined && (
                <div>
                  <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Log P</p>
                  <p className="font-medium">{data.xlogP.toFixed(2)}</p>
                </div>
              )}
              {data.tpsa !== undefined && (
                <div>
                  <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">TPSA</p>
            ...
            {/* Extended Properties */}
            {(data.hBondDonorCount !== undefined || data.hBondAcceptorCount !== undefined || data.rotatableBondCount !== undefined) && (
              <>
                <Separator />
                <div className="grid grid-cols-3 gap-2 text-center">
                  {data.hBondDonorCount !== undefined && (
                    <div className="bg-muted/30 rounded p-2">
                      <p className="text-lg font-bold">{data.hBondDonorCount}</p>
                      <p className="text-[9px] text-muted-foreground">H-Bond Donors</p>
                    </div>
                  )}
                  {data.hBondAcceptorCount !== undefined && (
                    <div className="bg-muted/30 rounded p-2">
                      <p className="text-lg font-bold">{data.hBondAcceptorCount}</p>
                      <p className="text-[9px] text-muted-foreground">H-Bond Acceptors</p>
                    </div>
                  )}
                  {data.rotatableBondCount !== undefined && (
                    <div className="bg-muted/30 rounded p-2">
                      <p className="text-lg font-bold">{data.rotatableBondCount}</p>
                      <p className="text-[9px] text-muted-foreground">Rotatable Bonds</p>
                    </div>
                  )}
                </div>
              </>
            ...
            {(data.meltingPoint || data.solubility) && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  {data.meltingPoint && (
                    <div>
                      <p className="text-[10px] text-muted-foreground">Melting Point</p>
                      <p className="text-xs line-clamp-1" title={data.meltingPoint}>{data.meltingPoint}</p>
                    </div>
                  )}
                  {data.solubility && (
                    <div>
                      <p className="text-[10px] text-muted-foreground">Solubility</p>
                      <p className="text-xs line-clamp-2" title={data.solubility}>{data.solubility}</p>
                    </div>
                  )}
                </div>
              </>
            ...
            {data.synonyms && data.synonyms.length > 0 && (
              <>
                ...
                <div className="space-y-1.5">
                  <p className="text-[10px] text-muted-foreground">Also Known As</p>
                  <div className="flex flex-wrap gap-1">
                    {data.synonyms.map((s, i) => (
                      <Badge key={i} variant="outline" className="text-[9px] px-1.5 py-0 h-4 font-normal">{s}</Badge>
                    ))}
                  </div>
                </div>
              </>
