...
export function RevenueProjections({ businessCases }: RevenueProjectionsProps) {
  ...
  const totalCOGS = businessCases.reduce(
    (sum, bc) => sum + (bc.total_cogs || 0),
    0,
  );
    ...
    <div className="space-y-4 sm:space-y-6">
      <Card>
        <CardHeader className="space-y-1.5">
          ...
          <CardDescription>
            Aggregated financial projections by fiscal year
          </CardDescription>
        </CardHeader>
      ...
      {hasScenarios && (
        <Card>
          <CardHeader className="space-y-1.5">
            ...
            <CardDescription>
              Business cases grouped by scenario
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="space-y-2">
              ...
              {(() => {
                const scenarioRevenue = businessCases.reduce(
                  (sum, bc) => sum + (bc.total_revenue || 0),
                  0,
                );
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0 space-y-0.5">
                      <p className="font-semibold text-sm">
                        ${(scenarioRevenue / 1000000).toFixed(2)}M
                      </p>
                      <p className="text-xs text-muted-foreground">
