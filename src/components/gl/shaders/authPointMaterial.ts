...
export class AuthDofPointsMaterial extends THREE.ShaderMaterial {
  constructor() {
    super({
      ...
      // Brand green color - Life Scientific olive green
      vec3 brandGreen = vec3(0.42, 0.56, 0.14); // #6B8E23 converted to normalized RGB
      vec3 darkGreen = vec3(0.33, 0.42, 0.18);  // #556B2F darker variant
      ...
      void main() {
        ...
        // Mix between dark green and brand green based on sparkle
        vec3 particleColor = mix(darkGreen, brandGreen, sparkleBrightness - 0.7);
        
        // Add slight color variation based on position for depth
        float colorVariation = periodicNoise(vInitialPosition * 2.0, uTime * 0.1);
        particleColor = mix(particleColor, brandGreen * 1.2, colorVariation * 0.3);
