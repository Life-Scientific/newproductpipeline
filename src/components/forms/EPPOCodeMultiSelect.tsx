...
export function EPPOCodeMultiSelect({
  ...
  const [allLoadedCodes, setAllLoadedCodes] = useState<Map<string, EPPOCode>>(new Map());
  ...
  const performSearch = useCallback(async (term: string) => {
        ...
        if (allLoadedCodes.size === 0) {
          searchEPPOCodes({
            ...
            if (result.data) {
              ...
              setAllLoadedCodes(new Map(filtered.map((code) => [code.eppo_code_id, code])));
              setSearchResults(filtered);
            }
          });
        } else {
          // Show all allowed codes from cache
          const filtered = Array.from(allLoadedCodes.values()).filter((code) =>
            allowedEppoCodeIds.includes(code.eppo_code_id)
          );
          setSearchResults(filtered);
        }
    ...
    try {
      ...
      if (result.data) {
        ...
        if (allowedEppoCodeIds && allowedEppoCodeIds.length > 0) {
          filtered = filtered.filter((code) => allowedEppoCodeIds.includes(code.eppo_code_id));
        }
  ...
  useEffect(() => {
    if (allowedEppoCodeIds && allowedEppoCodeIds.length > 0 && allLoadedCodes.size === 0 && !searchValue) {
      performSearch("");
    }
  ...
  const filteredOptions = useMemo(() => {
    return searchResults.filter((code) => !selected.includes(code.eppo_code_id));
          ...
          >
            <div className="flex flex-wrap gap-1 items-center">
              {selectedCodes.map((code) => (
                <Badge key={code.eppo_code_id} variant="secondary" className="mr-1 mb-1">
                  {getCodeLabel(code)}
                  <span
                    role="button"
                    tabIndex={0}
                  ...
                  >
                    <div className="flex-1 text-left">
                      ...
                      {code.latin_name && code.latin_name !== code.display_name && (
                        <div className="text-xs text-muted-foreground">{code.latin_name}</div>
                      )}
                    </div>
                  </button>
                );
