...
export function BusinessCaseImportPreview({ validations }: BusinessCaseImportPreviewProps) {
        ...
        <Table>
          ...
          <TableBody>
            {validations.map((validation) => {
              const { rowIndex, row, isValid, errors, warnings, resolved } = validation;
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    ...
                    isValid && warnings.length > 0 && "bg-amber-50/50 dark:bg-amber-950/20"
                  )}
                >
                  <TableCell className="font-mono text-sm">{rowIndex + 1}</TableCell>
                  <TableCell>
                    {isValid ? (
                  ...
                  <TableCell className="font-mono text-sm">{row.formulation_code}</TableCell>
                  <TableCell className="font-mono text-sm">{row.country_code}</TableCell>
                  <TableCell className="font-mono text-sm">{row.use_group_variant}</TableCell>
                  <TableCell className="text-sm">
                    {row.effective_start_fiscal_year || resolved?.target_market_entry_fy || "-"}
                  </TableCell>
                  <TableCell>
