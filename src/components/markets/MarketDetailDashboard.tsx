    ...
    const formulationDetails = Array.from(uniqueFormulations)
      .map((formId) => {
        const formulation = formulations.find(
          (f) => f.formulation_id === formId,
        );
        const formBCs = businessCases.filter(
          (bc) => bc.formulation_id === formId,
        );
        const formRevenue = formBCs.reduce(
          (sum, bc) => sum + (bc.total_revenue || 0),
          0,
        );
        const formMargin = formBCs.reduce(
          (sum, bc) => sum + (bc.total_margin || 0),
          0,
        );
        const formMarginPercent =
          formRevenue > 0 ? (formMargin / formRevenue) * 100 : 0;

        const name = formulation
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {metrics.approvedCount} approved, {metrics.submittedCount}{" "}
                submitted
              </p>
            </CardContent>
          </Card>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {reg.use_group_count && reg.use_group_count > 0 && (
                          <span className="text-xs text-muted-foreground">
                            {reg.use_group_count} use group
                            {reg.use_group_count > 1 ? "s" : ""}
                          </span>
                        )}
                      </div>
                    </div>
