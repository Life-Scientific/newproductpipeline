...
export function PipelineTrackerDashboard({
  ...
  const [selectedView, setSelectedView] = useState<"table" | "network">(
    "table",
  );
  ...
  const { processedData, globalStats } = useMemo(() => {
    const countriesByFormulation = new Map<
      string,
      FormulationCountryDetail[]
    >();
    countries.forEach((c) => {
      const code = c.formulation_code || "";
      if (!countriesByFormulation.has(code))
        countriesByFormulation.set(code, []);
    ...
    const enrichedFormulations: EnrichedFormulation[] = formulations.map(
      (f) => {
        ...
        const countryPenetration =
          fCountries.length > 0
            ? (activeCountries / fCountries.length) * 100
            : 0;
    ...
    const filtered = enrichedFormulations.filter((f) => {
      const status =
        "formulation_status" in f ? (f as any).formulation_status : null;
      const matchesStatus =
        statusFilters.size === 0 || (status && statusFilters.has(status));
      return matchesStatus;
    });

    const globalStats = {
      totalFormulations: filtered.length,
      totalCountries: filtered.reduce(
        (sum, f) => sum + f.stats.countryCount,
        0,
      ),
      totalActiveCountries: filtered.reduce(
        (sum, f) => sum + f.stats.activeCountryCount,
        0,
      ),
      totalUseGroups: filtered.reduce(
        (sum, f) => sum + f.stats.useGroupCount,
        0,
      ),
      totalBusinessCases: filtered.reduce(
        (sum, f) => sum + f.stats.businessCaseCount,
        0,
      ),
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            ...
            >
              <span
                className="font-semibold text-sm text-primary truncate max-w-[220px] group-hover/cell:underline"
                title={name || ""}
              >
                {name || "—"}
              </span>
              ...
              {activeIngredients && (
                <span
                  className="text-[10px] text-muted-foreground/70 truncate max-w-[220px]"
      ...
      },
      {
        id: "status",
        accessorFn: (row) =>
          "formulation_status" in row ? (row as any).formulation_status : null,
        header: ({ column }) => (
          <Button
            variant="ghost"
            size="sm"
        ...
        cell: ({ row }) => {
          const status =
            "formulation_status" in row.original
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const { activeCountryCount, countryCount, topCountries } =
            row.original.stats;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-medium text-sm">
            {row.original.stats.useGroupCount}
          </span>
        ),
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {STATUS_OPTIONS.map((status) => (
              ...
              >
                {status}
                {statusFilters.has(status) && <X className="ml-1 h-3 w-3" />}
              </Badge>
            ))}
            {hasActiveFilters && (
              <Button
          ...
          <div className="flex items-center gap-2">
            ...
            <Tabs
              value={selectedView}
              onValueChange={(v) => setSelectedView(v as "table" | "network")}
            >
          ...
          <Card className="bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800">
            <CardContent className="p-3">
              <p className="text-sm text-amber-900 dark:text-amber-100">
                Network view works best with ≤20 results. Filter or search to
                reduce results.
              </p>
            </CardContent>
          </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Card>
          <CardContent className="p-3">
            ...
            <div className="text-[10px] text-muted-foreground">
              {globalStats.totalActiveCountries} active
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="p-3">
            ...
            <div className="text-xl font-bold">
              {globalStats.totalBusinessCases}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            ...
            <div
              className={cn(
                "text-xl font-bold",
                globalStats.avgMarginPercent > 40
                  ? "text-green-600"
                  : "text-amber-600",
              )}
            >
