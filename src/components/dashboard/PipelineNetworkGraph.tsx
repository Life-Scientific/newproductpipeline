          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <div className="font-bold text-sm truncate" title={data.code}>
                  {data.code}
                </div>
                <div
            ...
            <Badge
              variant={getStatusVariant(data.status, "formulation")}
    ...
    >
      <CardContent className="p-3">
        <Handle
          ...
          className="!bg-slate-400 !w-2.5 !h-2.5"
        />
        <Handle
          type="source"
          position={Position.Right}
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge
              variant={getStatusVariant(data.status, "country")}
  ...
  const {
    ...
    const filteredFormulations = formulations.filter((f) => {
      const matchesSearch =
        (f.formulation_code?.toLowerCase().includes(searchTerm.toLowerCase()) ??
          false) ||
        (("formulation_name" in f ? (f as any).formulation_name : null)
          ?.toLowerCase()
          .includes(searchTerm.toLowerCase()) ??
          false);
      const matchesStatus =
        statusFilter === "all" ||
        ("formulation_status" in f
          ? f.formulation_status === statusFilter
          : false);
      return matchesSearch && matchesStatus;
    });

    // Limit to prevent performance issues - be more conservative with business cases
    const formulationsToShow =
      filteredFormulations.length > 10 && !searchTerm
    ...
    formulationsToShow.forEach((f) => {
      ...
      const fCountries = countries.filter(
        (c) => c.formulation_code === f.formulation_code,
      );
      let fRevenue = 0;
    ...
    nodes.forEach((node) => {
      if (node.type === "formulation" && node.data.revenue !== "€0") {
        ...
        const multiplier = node.data.revenue.includes("M")
          ? 1000000
          : node.data.revenue.includes("K")
            ? 1000
            : 1;
        const actualRevenue = revenue * multiplier;
        node.data.revenueShare =
          totalRevenue > 0
            ? Math.round((actualRevenue / totalRevenue) * 100)
            : 0;
      }
    });
      ...
      <Card className="h-[600px] flex flex-col items-center justify-center">
        <CardContent className="text-center space-y-2">
          <p className="text-muted-foreground">
            No items to display in network graph.
          </p>
    ...
    return (
      <Card className="h-[600px] flex flex-col items-center justify-center">
        <CardContent className="text-center space-y-4 max-w-md">
          ...
          <p className="text-muted-foreground">
            The network graph shows {initialNodes.length} nodes, which may
            impact performance.
          </p>
          <p className="text-sm text-muted-foreground">
            Please search for a specific formulation or use the table view for
            large datasets.
          </p>
        </CardContent>
      </Card>
      ...
      <CardContent className="p-0">
        <div className="h-[700px] relative">
          ...
          <div className="absolute bottom-4 left-4 bg-background/95 backdrop-blur border rounded-lg shadow-lg p-3 text-xs space-y-1">
            ...
            <div className="text-muted-foreground">Click nodes to navigate</div>
          </div>
        </div>
      </CardContent>
    </Card>
