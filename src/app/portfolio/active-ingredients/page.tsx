...
export default async function ActiveIngredientsPage() {
  ...
  const activeIngredients = allIngredients.filter((ing) => ing.ingredient_type === "Active");

  // Calculate summary statistics for active ingredients only
  const totalActiveIngredients = activeIngredients.length;
  const criticalRisk = activeIngredients.filter((ing) => ing.supply_risk === "Critical").length;
  const highRisk = activeIngredients.filter((ing) => ing.supply_risk === "High").length;
  const mediumRisk = activeIngredients.filter((ing) => ing.supply_risk === "Medium").length;
  const lowRisk = activeIngredients.filter((ing) => ing.supply_risk === "Low").length;
  const euApproved = activeIngredients.filter((ing) => ing.is_eu_approved).length;
      ...
      <AnimatedPage>
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            <h1 className="text-2xl sm:text-3xl font-bold">Active Ingredients</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium">Total Active Ingredients</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                {totalActiveIngredients > 0 ? ((euApproved / totalActiveIngredients) * 100).toFixed(1) : 0}% approved
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              View active ingredient usage across formulations, supply risk indicators, and supplier relationships
            </CardDescription>
          </CardHeader>
