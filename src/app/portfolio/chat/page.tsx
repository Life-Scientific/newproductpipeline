    ...
    async (text: string) => {
      ...
      try {
        ...
        while (reader) {
          ...
          setMessages((m) =>
            m.map((x) => (x.id === tempId ? { ...x, content: full } : x)),
          );
        }
        ...
        if (saved)
          setMessages((m) =>
            m.map((x) => (x.id === tempId ? (saved as Message) : x)),
          );
      } catch {
    ...
    <div className="h-full flex">
      ...
      >
        ...
        <div className="h-14 px-4 border-b flex items-center shrink-0">
          <Button
            variant="outline"
            className="w-full gap-2 justify-start h-9"
            onClick={newChat}
          >
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center p-8">
              ...
              <p className="text-muted-foreground text-center max-w-md mb-8">
                I can analyze your portfolio data from Supabase - formulations,
                business cases, ingredients, and financials.
              </p>
        ...
        <div className="border-t p-4 shrink-0">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs text-muted-foreground text-center mt-2">
              Data sourced from: vw_dashboard_summary,
              vw_formulations_with_ingredients, vw_ingredient_usage
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-3">
      {content.split("\n").map((line, i) => {
        ...
        if (line.startsWith("### "))
          return (
            <h4 key={i} className="font-semibold text-base mt-4 mb-2">
              {line.slice(4)}
            </h4>
          );
        if (line.startsWith("## "))
          return (
            <h3 key={i} className="font-semibold text-lg mt-5 mb-2">
              {line.slice(3)}
            </h3>
          );
        if (line.startsWith("# "))
          return (
            <h2 key={i} className="font-bold text-xl mt-5 mb-3">
              {line.slice(2)}
            </h2>
          );
        ...
        if (line.startsWith("|")) {
          return (
            <code
              key={i}
              className="block text-xs font-mono bg-muted/50 px-2 py-1 rounded"
            >
              {line}
            </code>
          );
